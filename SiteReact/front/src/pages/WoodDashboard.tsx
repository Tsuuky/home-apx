...
function buildRange(range: string, start: string, end: string) {
  if (range === "custom") {
    return { start, end };
  }
  const now = new Date();
...
export default function WoodDashboard() {
  ...
  const filteredSeries = useMemo(() => {
    ...
    const range = buildRange(rangeType, rangeStart, rangeEnd);
    return arr.filter((item) => item.date >= range.start && item.date <= range.end);
  }, [stats, rangeType, rangeStart, rangeEnd]);

  const lastDays = useMemo(() => {
    return filteredSeries.slice(-10).reverse();
  }, [filteredSeries]);
